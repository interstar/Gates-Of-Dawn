...
**Idea #2** : Files are managed inside _with makeFile()_ blocks.
...
**NB:** Don't try to reuse variables between one makeFile block and another. There's a weird implementation behind the scenes and it won't preserve connections between files. (Not that you'd expect it to.) 
...
Note that dac_ is an unusual function in Gates of Dawn that takes any number of signal arguments and combines them.
...
using it to make four of our simple oscillators defined previously : 
    ...
    with makeFile("hello_main.pd") as f :
        dac_(
...
In this example we have two "with makeFile()" blocks. The first defines a "hello.pd" file containing a single vol_osc() call. (This is the same vol_osc function we defined earlier.)
...
The second "with makeFile()" block defines a containing patch called hello_main.pd. It simply imports the hello.pd Abstraction 4 times and passes the four outputs into the dac_.
